...
object Main {
  ...
  def push[A](stk: Stack[A])(a: A): Stack[A] = MyCons(a, stk) // stk
  ...
  def enQ[A](q: Queue[A])(a: A): Queue[A] = (push(q._1)(a), q._2) // q
  ...
  {
    def moveQ(s1:Stack[A], s2:Stack[A]) : Queue[A] =
      pop(s1) match {
        case None => (s1, s2)
        case Some ((h, t)) => moveQ(t, push(s2)(h))
      }
  ...
  t match {
      ...
      case Node(kv, l, r) =>
        cmp(keyValue._1)(kv._1) match {
          case n if n < 0 =>
            Node(kv, insert(l)(keyValue)(cmp), r)
          case n if n > 0 =>
            Node(kv, l, insert(r)(keyValue)(cmp))
          case _ =>
            Node(keyValue, l, r)
        }
    }
  ...
  def lookup[K, V](t: BSTree[K, V])(key: K)(cmp: K => K => Int): Option[V] = // None
    t match {
      ...
      case Node(kv, l, r) =>
        cmp(key)(kv._1) match {
          case n if n < 0 => lookup(l)(key)(cmp)
          case n if n > 0 => lookup(r)(key)(cmp)
          case n if n == 0 => Some (kv._2)
        }
    }
  ...
  /*
   You can solve this without knowledge about 'class'.
   */
